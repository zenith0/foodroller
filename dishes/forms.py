...
class DishForm(forms.ModelForm):
    ...
    ingredients = forms.CharField(max_length=2000, widget=forms.Textarea(attrs={'placeholder': 'Ingredients', 'cols': 78, 'rows': 15}),
                                  required=True)
    ...
    cooking_time = forms.TimeField(label='Cooking time',
                                   widget=forms.TextInput(attrs={'placeholder': 'Cooking Time'}),
                                   required=True)
